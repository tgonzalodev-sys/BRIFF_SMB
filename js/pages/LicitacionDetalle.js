  ...
  return html`
    <div>
      ...
      <div style=${{ marginBottom: 20 }}>
        <button onClick=${() => navigate('/proveedores/licitaciones')} style=${{ background: 'none', border: 'none', color: '#6B7280', cursor: 'pointer', fontSize: 13, padding: 0, marginBottom: 12, fontFamily: 'inherit' }}>← Volver a Licitaciones</button>
      ...
      <div style=${{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                ...
                <tfoot>
                  <tr style=${{ borderTop: '2px solid #E5E7EB', background: '#F9FAFB' }}>
                    ...
                    ${invited.map(s => {
                      ...
                      return html`
                        <td key=${s.id} style=${{ padding: '12px 16px', textAlign: 'right', background: isAdjSupplier ? '#F0FDF4' : 'transparent' }}>
        ...
        <div style=${{ width: 260, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              ${invited.map(s => {
                ...
                return html`
                  <div key=${s.id} style=${{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    ${hasAdj && html`<span style=${{ fontSize: 11, fontWeight: 700, color: '#009966', background: '#F0FDF4', padding: '2px 8px', borderRadius: 99 }}>Adjudicado</span>`}
                  </div>
                `;
              })}
            </div>
          ...
          ${Object.keys(adj).length > 0 && html`
            <div style=${{ background: '#F0FDF4', borderRadius: 8, border: '1px solid #BBF7D0', padding: 18 }}>
